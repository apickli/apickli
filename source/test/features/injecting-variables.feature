...
#test variable injection logic
	...
	Scenario: using scenario variable
		Given I store the value bar as value1 in scenario scope
...
#test injection logic with other step definitions   
	...
	Scenario: Setting body payload using variables
        Given I store the value {"key":"hello-world"} as myPayload in scenario scope  
	...
	Scenario: Setting body payload from file using variable
        Given I store the value ./test/features/fixtures/requestbody.xml as myFilePath in scenario scope
	...
	Scenario: Sending request with basic auth authentication using variables
        Given I store the value username as myUsername in scenario scope
        And I store the value password as myPassword in scenario scope
	...
	Scenario: Response code checks
        Given I store the value 200 as myCode in scenario scope
        And I store the value 404 as myIncorrectCode in scenario scope
		When I GET /xml
	...
	Scenario: Response header value assertions
        Given I store the value Content-Type as myHeaderName in scenario scope
        And I store the value [a-z]/xml as myHeaderValue in scenario scope
        And I store the value boo as myHeaderIncorrectValue in scenario scope
		When I GET /xml
	...
	Scenario: Response body text assertions
        Given I store the value WonderWidgets as correctValue in scenario scope
        And I store the value boo as incorrectValue in scenario scope
		When I GET /xml
	...
	Scenario: Response body xpath assertions
        Given I store the value /slideshow/slide[2]/title as myPath in scenario scope
        Given I store the value [a-z]+ as correctValue in scenario scope
        And I store the value \d+ as incorrectValue in scenario scope
		When I GET /xml
	...
	Scenario: Access token retrieval from response body (authorization code grant, password, client credentials)
        Given I store the value $.headers.Token as myTokenPath in scenario scope
    ...
    Scenario: checking values of query parameter passed as datatable
        Given I store the value argument1 as myParam in scenario scope
        And I store the value test as myVal in scenario scope
    ...
    Scenario: should successfully validate json using schema
        Given I store the value ./test/features/fixtures/get-simple.schema as schemaPath in scenario scope
        When I GET /get
        Then response body should be valid according to schema file `schemaPath` 

    Scenario: should successfully validate json array
        Given I store the value 3 as myLength in scenario scope
